...
from drivlme.mm_utils import tokenizer_image_token, get_model_name_from_path
import argparse
import numpy as np
import os
import json
...
def drivlme_infer(video_frames, conversation, idx,last_out, conv_mode, model, vision_tower, tokenizer, image_processor, video_token_len):
    ...
    for i in range(idx):
        conv.append_message(conv.roles[1], conversation[1+2*i]["value"])
...
if __name__ == "__main__":
    ...
    for test in tqdm(test_data):
        ...
        if os.path.exists(video_path):
            ...
            print(f"Skipping: Video file does not exist: {video_path}")
            continue
        print(video_path)
    ...
    if not os.path.exists(out_dir):
        os.makedirs(out_dir)
    with open(os.path.join(out_dir,args.out_path),"w") as f:
        json.dump(outputs,f)
